...
export default class MemoryPool {
    ...
    async $load(url, handle) {
        ...
        //
        try {
            const fs = (await import("indexeddb-fs")).default;
    ...
    $wrap($c, what, TA = Uint32Array) {
        ...
        const _val_ = codec._calloc(1, TA.BYTES_PER_ELEMENT||1);
        const _ptr_ = new Proxy(codec, new MemoryHandler({$ptr: _val_, $typed: TA, $length: length}));//new TA(codec.HEAP8.buffer, codec.HEAP8.byteOffset + _val_, 1);
        _ptr_[0] = what?.byteOffset ?? what;
    ...
    $calloc($c, length = 4, TA = Uint8Array) {
        ...
        const _ptr_ = new Proxy(codec, new MemoryHandler({$ptr: _val_, $typed: TA, $length: length}));//new TA(codec.HEAP8.buffer, codec.HEAP8.byteOffset + _val_, (length||4));
        this.#registry.register(_ptr_, {"$ptr": _val_, "$codec": $c });
        return _ptr_;
    }
