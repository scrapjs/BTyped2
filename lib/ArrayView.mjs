...
export default class ArrayView {
    ...
    $get($index) {
        ...
        // get primitive
        const $target = new DataView(this.$target, $index * $memT.$byteLength + this.#byteOffset);
        return $target["get" + ($name.includes("int64") ? "Big" : "") + ($name.charAt(0).toUpperCase() + $name.slice(1))](0, true);
    }
    ...
    $set($index, $member = 0) {
        ...
        {
            const $target = new DataView(this.$target, $index * $memT.$byteLength + this.#byteOffset);
            $target["set" + ($name.includes("int64") ? "Big" : "") + ($name.charAt(0).toUpperCase() + $name.slice(1))](0, $member, true); 
        };
        return true;
    }
};
