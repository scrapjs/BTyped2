...
export default class StructView {
    ...
    $get($name) {
        ...
        if (!isNaN($index)) {
            return new Proxy(this.#layout.$view(this.#target, (this.#layout.$byteLength * $index + this.#byteOffset), (this.#length - $index)), new ProxyHandle(this.#layout));
        }

        // direct typing
        if (typeof $type.$name == "object") {
            const $T = $type.$name;
            return new Proxy($T.$view(this.#target, (this.#byteOffset + $type.$offset), ($type.$array || 1)), new ProxyHandle($T));
        }

        // if structure member
        if (CStructs[$type.$name]) {
            return new Proxy(CStructs[$type.$name].$view(this.#target, (this.#byteOffset + $type.$offset), ($type.$array || 1)), new ProxyHandle(CStructs[$type.$name]));
        }

        //
        if (CTypes[$type.$name] && $type.$array) { 
            return new Proxy(CTypes[$type.$name].$view(this.#target, (this.#byteOffset + $type.$offset), ($type.$array || 1)), new ProxyHandle(CTypes[$type.$name]));
        }
