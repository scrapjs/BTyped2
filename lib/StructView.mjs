...
export default class StructView {
    ...
    $get($name) {
        ...
        return $target["get" + ($type.$name?.includes?.("int64") ? "Big" : "") + ($type.$name.charAt(0).toUpperCase() + $type.$name.slice(1))](this.#byteOffset + $type.$offset, true);
    }
    ...
    $set($name, $member = 0) {
        ...
        {
            const $target = (this.$target instanceof DataView) ? this.$target : new DataView(this.$target, 0);
            $target["set" + ($type.$name.includes("int64") ? "Big" : "") + ($type.$name.charAt(0).toUpperCase() + $type.$name.slice(1))](this.#byteOffset + $type.$offset, $member, true); 
        };
