...
export default class StructView {
    ...
    $get($name) {
        ...
        // get primitive
        const $target = new DataView(this.$target, this.#byteOffset + $type.$offset);
        return $target["get" + ($type.$name?.includes?.("int64") ? "Big" : "") + ($type.$name.charAt(0).toUpperCase() + $type.$name.slice(1))](0, true);
    }
    ...
    $set($name, $member = 0) {
        ...
        {
            const $target = new DataView(this.$target, this.#byteOffset + $type.$offset);
            $target["set" + ($type.$name.includes("int64") ? "Big" : "") + ($type.$name.charAt(0).toUpperCase() + $type.$name.slice(1))](0, $member, true); 
        };
