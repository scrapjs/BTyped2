...
export default class StructView {
    ...
    $typeof($name, $mT = null) {
        ...
        return (($mT instanceof StructType) ? new StructType($mT, $index) : new StructType($mT, $offset, $array, $default, $index));
    }
    ...
    $get($name) {
        ...
        let $T = $type.$name;
        if (CStructs[$T]                ) { $T = CStructs[$T]; };
        if (CTypes  [$T] && $type.$array) { $T = CTypes  [$T]; };
